...
@tf_export("keras.initializers.PositiveVarianceScaling",
           "initializers.positive_variance_scaling", "positive_variance_scaling_initializer")
class PositiveVarianceScaling(Initializer):
  """Initializer capable of adapting its scale to the shape of weights tensors.
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    if self.distribution == "normal":
      stddev = math.sqrt(scale)
      return random_ops.truncated_normal(
          shape, 2*stddev, stddev, dtype, seed=self.seed)
    else:
      limit = math.sqrt(3.0 * scale)
      return random_ops.random_uniform(
          shape, 0, 2*limit, dtype, seed=self.seed)
...
@tf_export("positive_glorot_uniform_initializer")
def positive_glorot_uniform_initializer(seed=None, dtype=dtypes.float32):
  """The Glorot uniform initializer, also called Xavier uniform initializer.
  ...
  Returns:
    An initializer.
  """
  return PositiveVarianceScaling(
      scale=1.0, mode="fan_avg", distribution="uniform", seed=seed, dtype=dtype)


@tf_export("positive_glorot_normal_initializer")
def positive_glorot_normal_initializer(seed=None, dtype=dtypes.float32):
  """The Glorot normal initializer, also called Xavier normal initializer.
